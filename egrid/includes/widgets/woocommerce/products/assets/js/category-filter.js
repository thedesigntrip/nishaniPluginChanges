(function ($) {
  ...
  var WidgetEGridProductsCategoryFilterHandler = function ($scope, $) {
    ...
    if (els.length == 0) {
      ...
      els = $("#egrid-products-filters-" + widgetId).find(
        "[egrid-products-category-filter]"
      );
    }
      ...
      if (el.is("select")) {
        ...
        el.selectWoo({
          ...
          matcher: function (params, data) {
            ...
            if (
              data.text.toLowerCase() === "uncategorised" ||
              data.text.toLowerCase() === "uncategorized"
            ) {
              return null;
            }
  ...
  $(window).on("elementor/frontend/init", function () {
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/egrid-products.default",
      WidgetEGridProductsCategoryFilterHandler
    );
  });
})(jQuery);
